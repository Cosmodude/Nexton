...
export class JettonMinter implements Contract {
    ...
    static mintMessage(to: Address, query_id: bigint, jetton_amount: bigint, forward_ton_amount: bigint, from: Address, total_ton_amount: bigint) {
        const mintMsg = beginCell().storeUint(0x178d4519, 32)
                                   .storeUint(query_id, 64)
                                   .storeCoins(jetton_amount)
                                   .storeAddress(null) // from addr
                                   .storeAddress(from) // Response addr
                                   .storeCoins(forward_ton_amount)
                                   .storeMaybeRef(null)
                    .endCell();

        return beginCell().storeUint(0x15, 32).storeUint(query_id, 64) // op, queryId
                          .storeAddress(to)
    ...
    async sendMint(provider: ContractProvider, via: Sender, to: Address, query_id: bigint, jetton_amount: bigint, forward_ton_amount: bigint, total_ton_amount: bigint,) {
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: JettonMinter.mintMessage(to, query_id, jetton_amount, forward_ton_amount, this.address, total_ton_amount),
